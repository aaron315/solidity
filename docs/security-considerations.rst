...
Also try to enable the "0.5.0" safety features as early as possibly
by adding ``pragma experimental "v0.5.0";``. Note that the word
``experimental`` does not mean that the safety features are in any
